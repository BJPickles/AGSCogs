...
class AGSServerStatus(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        default_global = {
            "servers": {},          # Stored as {name: {"ip": ip, "port": port, "enabled": bool, "last_status": bool or None}}
        ...
        # In-memory storage (last_status is now also persisted)
        # Each server is stored as a 4-tuple: (ip, port, last_status, enabled)
        self.servers = {}
        self.status_channel = None
        self.status_messages = {}
        self.active = True
        # New dict to track the last update message per realm.
        self.last_messages = {}
    ...
    async def initialize_settings(self):
        ...
        for name, details in servers.items():
            ...
            # Load “last_status” from storage (or None if it isn’t present).
    ...
    # Helper method to send a status update message while deleting the previous one (if any) for the same realm.
    async def _send_status_update(self, realm: str, channel: discord.TextChannel, content: str):
    ...
    async def add(self, ctx, name: str, ip: str, port: int):
        ...
        # Save the new server details including a blank "last_status".
    ...
    async def remove(self, ctx, name: str):
        ...
        if name in self.servers:
            ...
            # Also remove any stored update message for that realm.
    ...
    async def togglerealm(self, ctx, name: str, state: str = None):
        ...
        if new_enabled and self.status_channel:
            # When enabling, perform an immediate status check – but only send an update if the state changes.
            new_status_bool = await self.is_server_online(ip, port)
            if last_status is not None and new_status_bool == last_status:
                # No change in state: do not post an update.
                return
            ...
            # Save the new status in memory and persistent config.
    ...
    async def setmessage(self, ctx):
        ...
        To set a custom message, provide a text argument or reply with one:
    ...
    async def run_status_update(self):
        """Immediately check status for all servers (only enabled realms) and send updates if the state has changed."""
        ...
        for name, (ip, port, last_status, enabled) in list(self.servers.items()):
            ...
            if last_status is None or new_status != last_status:
                ...
                try:
                    ...
                    message = default_message
                await self._send_status_update(name, channel, message)
                # Update in-memory value and persistent config.
    ...
    async def instructions_generic(self, ctx):
        ...
        and that your firewall is configured to allow inbound connections on the specified port.
        """
        message = (
